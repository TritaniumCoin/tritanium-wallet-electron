...
function updateSyncProgres(data){
    ...
    if(knownBlockCount === SYNC_STATUS_NET_CONNECTED){
        ...
        if(connNodeFee > 0 ){
            connStatusText += ` | Node fee: <strong>${connNodeFee.toFixed(6)} TTNZ</strong>`;
        }
...
function updateBalance(data){
    ...
    let bUnlocked = (availableBalance / 1000000).toFixed(6);
    let bLocked = (data.lockedAmount / 1000000).toFixed(6);
    ...
    let wintitle = `(${walletFile}) - ${bUnlocked} TTNZ`;
    setWinTitle(wintitle);
    
    if(availableBalance > 0){
        let maxSend = (bUnlocked - (wsession.get('nodeFee')+0.0001)).toFixed(6);
...
function showFeeWarning(fee){
    ...
    let htmlStr = `
        ...
        <p>The fee for sending transactions is: <strong>${fee.toFixed(6)} TTNZ </strong>.<br>
